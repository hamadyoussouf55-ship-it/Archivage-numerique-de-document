...
def make_base():
    ent  = Entreprise.objects.create(nom='TEST SAS', siret='99999999999999', code='TST')
...
class ArmoireModelTests(TestCase):
    ...
    def test_armoire_str(self):
        arm = Armoire.objects.create(nom='Armoire A', code='ARM-A', entreprise=self.ent)
        self.assertIn('Armoire A', str(arm))

    def test_rayon_str(self):
        arm = Armoire.objects.create(nom='Armoire B', code='ARM-B', entreprise=self.ent)
        ray = Rayon.objects.create(code='R01', niveau=1, armoire=arm)
        self.assertIn('R01', str(ray))

    def test_rayon_belongs_to_armoire(self):
        arm = Armoire.objects.create(nom='Armoire C', code='ARM-C', entreprise=self.ent)
