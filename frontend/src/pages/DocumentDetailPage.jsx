...
import { ArrowLeft, FileText, Download, Trash2, Move, Tag, Calendar, User, MapPin, Hash, Clock, CheckCircle, Archive, X, Save, Plus, Eye } from 'lucide-react'
...
function DeplacerModal({ docId, onClose, onSuccess }) {
  ...
  const mutation = useMutation(() => documentsAPI.deplacer(docId, rayon), {
...
function MetadataEditor({ docId, metadata, onSave }) {
  ...
  const mutation = useMutation(
    (data) => documentsAPI.updateMetadata(docId, { ...data, mots_cles: motsCles }),
...
export default function DocumentDetailPage() {
  ...
  const { data: doc, isLoading } = useQuery(['document', id], () => documentsAPI.get(id).then(r => r.data))
  ...
  const handleDownload = async () => {
    setDownloading(true)
    try { await documentsAPI.download(id, doc?.nom_fichier); toast.success('Téléchargement démarré') }
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      const r = await fetch(`/api/documents/${id}/previsualiser/`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  return (
    <div className="p-6 space-y-5">
      {showDeplacer && <DeplacerModal docId={id} onClose={() => setShowDeplacer(false)}
        ...
        <div className="flex items-center gap-2 flex-shrink-0 flex-wrap">
          ...
          {canWrite() && (
            <>
              ...
              <button onClick={() => { if (confirm('Supprimer ?')) deleteMutation.mutate() }} className="btn-danger text-sm py-1.5 px-3"><Trash2 size={14} /></button>
            </>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="card">
            <div className="flex items-center justify-between mb-4">
              ...
              {canWrite() && (
                <button onClick={() => setEditMeta(!editMeta)} className="text-xs font-semibold transition-colors" style={{ color: '#2563eb' }}>
                  {editMeta ? 'Annuler' : 'Modifier'}
                </button>
              )}
            </div>
            {editMeta ? (
              <MetadataEditor docId={id} metadata={doc.metadata} onSave={() => { setEditMeta(false); qc.invalidateQueries(['document', id]) }} />
            ) : (
              <div className="grid grid-cols-2 gap-4">
                {[
        ...
        <div className="space-y-4">
          <div className="card">
            ...
            <div className="h-40 rounded-lg flex flex-col items-center justify-center gap-3"
              ...
              <p className="text-xs" style={{ color: '#94a3b8' }}>
                {doc.nom_fichier?.endsWith('.pdf') ? 'PDF — cliquez pour ouvrir' : 'Téléchargez pour voir'}
              </p>
          ...
          <div className="card">
            <h2 className="section-title mb-3 flex items-center gap-2">
              <Clock size={14} style={{ color: '#2563eb' }} /> Historique
            </h2>
            {!doc.journal?.length ? (
              <p className="text-xs" style={{ color: '#94a3b8' }}>Aucune action enregistrée</p>
            ) : (
              <div className="space-y-3">
                {doc.journal.slice(0, 8).map((action, i) => (
                  <div key={i} className="flex items-start gap-2.5">
                    <div className="w-2 h-2 rounded-full mt-1.5 flex-shrink-0" style={{ background: '#3b82f6' }} />
                    <div>
                      <p className="text-xs font-semibold" style={{ color: '#1e293b' }}>{action.type_action}</p>
                      <p className="text-xs mt-0.5" style={{ color: '#94a3b8' }}>
                        {action.auteur_nom} · {action.date_action ? format(new Date(action.date_action), 'dd MMM yyyy HH:mm', { locale: fr }) : ''}
                      </p>
                    </div>
                  </div>
                ))}
