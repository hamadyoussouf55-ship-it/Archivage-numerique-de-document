...
function RayonModal({ armoireId, onClose, onSuccess }) {
  ...
  const mutation = useMutation(
    (data) => armoiresAPI.createRayon({ ...data, armoire: armoireId }),
    { onSuccess: () => { toast.success('Rayon créé !'); onSuccess() }, onError: () => toast.error('Erreur') }
  )
  return (
      ...
      <div className="card w-full max-w-md shadow-xl">
        ...
        <form onSubmit={handleSubmit(d => mutation.mutate(d))} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><label className="label">Code *</label><input className="input font-mono" {...register('code', { required: true })} /></div>
...
export default function ArmoireDetailPage() {
  ...
  const { data: armoire, isLoading } = useQuery(['armoire', id], () => armoiresAPI.get(id).then(r => r.data))
  const { data: documents } = useQuery(['documents', 'rayon', selectedRayon],
    () => documentsAPI.list({ rayon: selectedRayon }).then(r => r.data), { enabled: !!selectedRayon })
  const deleteRayon = useMutation((rid) => armoiresAPI.deleteRayon(rid), {
    onSuccess: () => { toast.success('Rayon supprimé'); qc.invalidateQueries(['armoire', id]) },
    onError: () => toast.error('Impossible (documents liés ?)'),
  })

  if (isLoading) return <div className="p-6"><div className="h-48 rounded-xl animate-pulse" style={{ background: '#e2e8f0' }} /></div>

  return (
    <div className="p-6 space-y-5">
      {showModal && <RayonModal armoireId={id} onClose={() => setShowModal(false)}
      ...
      <div className="flex items-center gap-4">
        ...
        {canWrite() && <button onClick={() => setShowModal(true)} className="btn-primary ml-auto"><Plus size={16} /> Nouveau rayon</button>}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-1 space-y-2">
          <h2 className="section-title flex items-center gap-2">
            <Layers size={16} style={{ color: '#2563eb' }} /> Rayons ({armoire?.rayons?.length ?? 0})
          </h2>
          {armoire?.rayons?.map(rayon => (
            <button key={rayon.id} onClick={() => setSelectedRayon(rayon.id === selectedRayon ? null : rayon.id)}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      {rayon.position && <span className="text-xs" style={{ color: '#94a3b8' }}><MapPin size={9} className="inline" /> {rayon.position}</span>}
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-xs flex items-center gap-1" style={{ color: '#94a3b8' }}>
                    <FileText size={11} /> {rayon.nombre_documents}
                  </span>
                  {canWrite() && (
                    <button onClick={e => { e.stopPropagation(); deleteRayon.mutate(rayon.id) }}
          ...
          {armoire?.rayons?.length === 0 && <p className="text-sm text-center py-4" style={{ color: '#94a3b8' }}>Aucun rayon</p>}
        </div>
            ...
            <div className="card">
              <div className="flex items-center justify-between mb-4">
                <h2 className="section-title">
                  Rayon <span style={{ color: '#2563eb' }}>{armoire?.rayons?.find(r => r.id === selectedRayon)?.code}</span>
                </h2>
                {canWrite() && <Link to={`/documents/nouveau?rayon=${selectedRayon}`} className="btn-primary text-xs py-1.5 px-3"><Plus size={13} /> Ajouter</Link>}
              </div>
              {!documents
                ? <div className="h-12 animate-pulse rounded-lg" style={{ background: '#f1f5f9' }} />
                : documents.results?.length === 0
                ? <div className="text-center py-10" style={{ color: '#94a3b8' }}><FileText size={28} className="mx-auto mb-2 opacity-30" /><p className="text-sm">Aucun document</p></div>
